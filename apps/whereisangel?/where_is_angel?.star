...
def main(config):
    ump_id = config.str("ump", DEFAULT_UMP)
    ump_name = UMPS[ump_id].upper()
    url = MLB_BASE_URL.format(MLB_SCHED_ENDPOINT)
    query_params = {
        "sportId": "1",
        "hydrate": "hydrations,officials,probableOfficials"
    }
    response = http.get(url, params = query_params, ttl_seconds = 600)
    print(response.url)

    # loop over officials to figure out if our ump is present in each game
    ump_game = None
    for game in response.json().get("dates")[0].get("games"):
        for official in game.get("officials"):
            if ump_id == str(int(official.get("official").get("id"))):
                ump_game = game
                ump_location = official.get("officialType")
                break
        # umps can't be in two places at once! or can they???
        if ump_game != None:
            break

    # if we have found the ump, figure out where he is
    away_team_id = None
    home_team_id = None
    if ump_game != None:
        print(ump_location)
        away_team_id = int(ump_game.get("teams").get("away").get("team").get("id"))
        home_team_id = int(ump_game.get("teams").get("home").get("team").get("id"))
    frames = []
    ...
    return schema.Schema(
        ...
        fields = [
            schema.Dropdown(
                id = "ump",
                name = "Umpire",
                desc = "MLB ump to track down.",
                icon = "baseballBatBall",
                options = ump_options,
                default = "427220",  # ANGEL HERNANDEZ
            ),
        ],
    )
