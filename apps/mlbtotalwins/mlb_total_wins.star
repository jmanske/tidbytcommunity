...
DEFAULT_TEAM = "158"
TEAM_FONT = "CG-pixel-3x5-mono"

def main(config):

    team_to_follow = config.str("team", DEFAULT_TEAM)
    
    date = get_date(config)

    timezone = get_timezone(config)
    
    # get the total number of games today
    num_games = int(sched.get("totalGames"))
    
    # if there aren't any, find the next game
    season = time.now().year
    #if num_games == 0:
    if 0 == 0:
        team_id = int(team_to_follow)
        return render.Root(
            child = render_future_game(season, team_id, timezone)
        )

    home_team_id = get_today_game_home_team_id(sched)
    away_team_id = get_today_game_away_team_id(sched)
    game_pk = get_today_game_pk(sched)

    current_data = get_probability_data(game_pk)
    home_team_prob = get_current_home_team_prob(current_data)
    ...
    return render.Root(
        ...
        child = render.Stack(
            children = [
                render_header(home_team_id, away_team_id, home_team_prob),
                get_plot(game_pk, home_team_id, away_team_id)
            ]
        )
    )
    ...
    return schema.Schema(
        ...
        fields = [
            ...
            schema.DateTime(
                id = "sched_date",
                name = "Schedule Date",
                desc = "The day to display information for",
                icon = "gear"
            ),
            schema.Location(
                id = "location",
...
def render_future_game(season, team_id, timezone):
    # get end date for current season
    url = "https://statsapi.mlb.com/api/v1/seasons/{season}?sportId=1".format(season = str(season))
    season_end_date = http.get(url, ttl_seconds=3600).json().get("seasons")[0].get("postSeasonEndDate")
    
    # find the next game, starting with today
    today = get_today_date()
    today = "2023-07-27"
    url = "http://statsapi.mlb.com/api/v1/schedule/games/?sportId=1&teamId={teamId}&startDate={startDate}&endDate={endDate}&fields=dates,games,gameDate,teams,team,id,leagueRecord,wins,losses".format(teamId=team_id, startDate=today, endDate=season_end_date)
    sched_obj = http.get(url, ttl_seconds=3600).json().get("dates")[0]
    game = get_game_object(sched_obj.get("games")[0], team_id, timezone)

    away_team_id = game.OpponentTeamId if game.IsHome else team_id
    home_team_id = team_id if game.IsHome else game.OpponentTeamId
    return render.Column(
        children = [
            render.Box(
                ...
                child = render.Text(
                    font = TEAM_FONT,
                    content = "next game " + game.LocalStartTime.StartDate,
            ...
            render.Box(
                ...
                child = render.Text(
                    font = TEAM_FONT,
                    content = "at " + game.LocalStartTime.StartTime,
                    color = TEAM_INFO[team_id].ForegroundColor
                )
            ),
            render.Row(
                children = [
                    render_logo_size(game.OpponentTeamId if game.IsHome else team_id, 19),
                    render.Column(
                        children = [
                            render.Box(
                                width = 26,
                                height = 7,
                                color = TEAM_INFO[away_team_id].BackgroundColor,
                                child = render.Text(
                                    font = TEAM_FONT,
                                    color = TEAM_INFO[away_team_id].ForegroundColor,
                                    content = game.OpponentRecord if game.IsHome else game.Record
                                )
                            ),
                            render.Box(
                                width = 26,
                            ...
                            render.Box(
                                width = 26,
                                height = 7,
                                color = TEAM_INFO[home_team_id].BackgroundColor,
                                child = render.Text(
                                    font = TEAM_FONT,
                                    color = TEAM_INFO[home_team_id].ForegroundColor,
                                    content = game.Record if game.IsHome else game.OpponentRecord
                                )
                            )
                        ]
                    ),
                    render_logo_size(team_id if game.IsHome else game.OpponentTeamId, 19)
                ]
            )         
        ]
    )

def get_game_object(game_data_raw, team_id, timezone):
    teams = game_data_raw.get("teams")
    start = convert_time_to_local(game_data_raw.get("gameDate"), timezone)
    if int(teams.get("away").get("team").get("id")) == team_id:
        is_home = False
        record = extract_record(teams, "away")
        opp_team_id = extract_team_id(teams, "home")
        opp_record = extract_record(teams, "home")
    else:
        is_home = True
        record = extract_record(teams, "home")
        opp_team_id = extract_team_id(teams, "away")
        opp_record = extract_record(teams, "away")
    return struct(IsHome = is_home, Record = record, OpponentTeamId = opp_team_id, OpponentRecord = opp_record, LocalStartTime = start)

def convert_time_to_local(dateTime, timezone):
    local_time = time.parse_time(dateTime).in_location(timezone)
    ...
    local_start_time = str(hour) + ":" + str(minute) + am_pm
    local_start_date = str(local_time.month) + "/" + str(local_time.day)
    return struct(StartDate = local_start_date, StartTime = local_start_time)

def get_today_date():
    now = time.now()
    return DATE_FORMAT.format(year=now.year, month=now.month, day=now.day)
...
def get_today_sched_object(team_id,date):
    url = MLB_SCHEDULE_URL
    query_params = {"sportId": "1", "teamId": str(team_id), "date": date}
    return http.get(url, params = query_params).json()

def get_today_game_pk(sched):
    return int(sched.get("dates")[0].get("games")[0].get("gamePk"))

def get_today_game_home_team_id(sched):
    return get_today_game_team_id(sched, "home")

def get_today_game_away_team_id(sched):
    return get_today_game_team_id(sched, "away")

def get_today_game_team_id(sched, type):
    return int(sched.get("dates")[0].get("games")[0].get("teams").get(type).get("team").get("id"))


def render_logo_size(team_id, size):
    return render.Image(
        src = TEAM_INFO[team_id].Logo,
        width = size
    )

def render_team_header(team_id):
    return render.Box(
        width=15,
        height=7,
        color = TEAM_INFO[team_id].BackgroundColor,
        child = render.Text(
            content = TEAM_INFO[team_id].Abbreviation,
            font = TEAM_FONT,
            color = TEAM_INFO[team_id].ForegroundColor
        )
    )
