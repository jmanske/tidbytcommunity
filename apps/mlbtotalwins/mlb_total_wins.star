"""
Applet: MLB Total Wins
Summary: Show cumulative wins
Description: Shows the graph of cumulative wins for the given MLB team.
Author: Jake Manske
"""
...
def main(config):
    ...
    plots = get_win_graph_widgets(team_id, season)

    return render.Root(
        delay = 75,
        child = render.Stack(
            children = [
                render_header(season, team_id),
                render.Animation(
                    children = plots
                ),
                render_adornment(team_id)
            ]
        )
    )

def render_header(season, team_id):
    abbrev = TEAM_INFO[team_id].Abbreviation
    team_name_length = len(abbrev)
    return render.Stack(
        children = [
            render.Box(
                width = 64,
            ...
            render_header_element(abbrev, 1, team_id),
            render_header_element(season, 4 * team_name_length + 3, team_id),
            render_header_element("wins", 4 * team_name_length + 21, team_id),
        ]
    )

def render_header_element(content, offset, team_id):
    return render.Row(
            children = [
                render.Box(
                    width = offset,
...
def format_date(timestamp):
    return DATE_FORMAT.format(year=timestamp.year, month=timestamp.month, day=timestamp.day)

def get_win_graph_dates(season):
    url = MLB_SEASONS_URL.format(str(season))
    query_params = { 
        "sportId": "1", 
        "fields": "seasons,regularSeasonStartDate,regularSeasonEndDate" 
        }
    
    # get season data, cache for 1 hour
    response = http.get(url, params = query_params, ttl_seconds = 3600)
    
    if is_response_OK(response):
        season_data = response.json().get("seasons")[0]
        start_date = season_data.get("regularSeasonStartDate")
        end_date = season_data.get("regularSeasonEndDate")
        valid = True
    else:
    ...
    return struct(Valid = valid, StartDate = start_date, EndDate = end_date)

def is_response_OK(request):
    return request.status_code == HTTP_OK

def get_date(config):
    config_time = config.str("sched_date")
    if config_time != None:
        now = time.parse_time(config.str("sched_date"))
    else:
        now = time.now()
    date = DATE_FORMAT.format(year=now.year, month=now.month, day=now.day)
    return date
...
def get_win_graph_widgets(team_id, season):
    dates = get_win_graph_dates(season)

    # TODO: if the http request failed, we will have set this struct to have invalid dates
    # TODO: return something to display here in case of failure
    if not dates.Valid:
        print("lol")
        
    # parse the dates we are given to figure out what kind of display we want
    begin = time.parse_time(dates.StartDate, FORMAT_STRING)
    end = time.parse_time(dates.EndDate, FORMAT_STRING)
    if (end - time.now()).minutes > 0:
        end = time.now()

    # call into MLB API for results
    results = http_get_sched_results(team_id, begin, end)

    # figure out how many wins and losses the team has right now so we can scale the graph
    # the latest date in the results will have the current record
    team_current_record = get_current_record(results, team_id)
    current_total_wins = team_current_record.Wins
    y_limit = current_total_wins if current_total_wins > 25 else 25

    # base total games on current record, not on the "totalGames" item
    # there may have been postponed/resumed games and we cannot count them twice
    total_games = team_current_record.Wins + team_current_record.Losses
    x_limit = total_games if total_games > 64 else 64

    # loop through results and build graph  
    wins_array = []
    widgets = []
    i = 0
    wins = 0
    for date in results.get("dates"):
        for game in date.get("games"):
            if is_game_final(game):
                # figure out if the team we are following won
                if did_team_win_game(game, team_id):
                    wins += 1
                wins_array.append((i, wins))
                i += 1
            elif is_game_postponed(game):
                continue
            # adding copies to the end will make the animation "pause" when the graph is complete
            copies_to_add = 50 if i == total_games else 1
            for j in range(copies_to_add):
                widgets.append(
                    render.Column(
                        children = [
                            render_header_element(wins, 64 - 4 * len(str(wins)), team_id),        
                            render.Box(
                                width = 1,
                                height = 1
                            ),
                            render.Plot(
                                ...
                                color = TEAM_INFO[team_id].ForegroundColor,
                                fill = True,
                                fill_color = get_fill_color(team_id)
                            )
                        ]
                    )
                )
...
def is_game_final(game):
    # we will consider the game in "final" status if it is coded as final
    # AND it does not have a "resume date" differing from the official date of the game
    # if it does, we may count it twice, so we skip it if it is in this state
    status = game.get("status").get("codedGameState")
    if status == "F":
        resume_date = game.get("resumeGameDate")
        if (resume_date != None) and (resume_date != game.get("officialGameDate")):
            return False
        return True
    return False
...
def is_team_away(game, team_id):
    if int(game.get("teams").get("away").get("team").get("id")) == team_id:
        return True
    return False

def did_team_win_game(game, team_id):
    if is_team_away(game, team_id):
        if game.get("teams").get("away").get("isWinner"):
            return True
    else:
        if game.get("teams").get("home").get("isWinner"):
            return True
    return False

def get_current_record(results, team_id):
    total_dates = len(results.get("dates"))
    game = results.get("dates")[total_dates - 1].get("games")[0]
    if is_team_away(game, team_id):
        wins = int(game.get("teams").get("away").get("leagueRecord").get("wins"))
        losses = int(game.get("teams").get("away").get("leagueRecord").get("losses"))
    else:
        wins = int(game.get("teams").get("home").get("leagueRecord").get("wins"))
        losses = int(game.get("teams").get("home").get("leagueRecord").get("losses"))
    return struct(Wins = wins, Losses = losses)

def http_get_sched_results(team_id, start_date, end_date):
    query_params = {
        "sportId": "1",
        "fields": "totalItems,totalGames,statusCode,detailedState,dates,date,games,status,codedGameState,teams,team,id,isWinner,leagueRecord,wins,losses,gamePk,officialDate,resumeGameDate",
        "teamId": str(team_id),
        "startDate": format_date(start_date),
        "endDate": format_date(end_date)
    }
    response = http.get(MLB_SCHEDULE_URL, params = query_params, ttl_seconds = 1)
    # TODO: check status_code and possibly Tidbyt-Cache-Status
    if is_response_OK(response):
